...
namespace vjac
{
    public class VJudgeOperator
    {
        ...
        async Task<string> GetStatusStringAsync(string UserName, int startPos)
        {
            ...
            crawler.Content = string.Format(VJ_STATUS_FORM_URL, startPos, UserName, ContestID);
            crawler.Cookies = cookies;
            return await crawler.PostFormAsync();
        }

        public async Task<List<Status>> GetStatusAsync(string UserName = "", string TilRID = "")
        {
            List<Status> list = new List<Status>();
            int startPos = 0;
            do
            {
                string orig = await GetStatusStringAsync(UserName, startPos);
                ...
                foreach (JsonValue i in obj["data"])
                {
                    ...
                    if (log["statusType"].ToString() == "0")
                    {
                        var s = new Status()
                        {
                            ...
                            Problem = log["contestNum"].ToString()[1].ToString(),
                        };
                        if (s.RID == TilRID)
                        {
                            return list;
                ...
                if ((obj["data"] as JsonArray).Count < 20)
                {
                    break;
                }
                startPos += 20;
